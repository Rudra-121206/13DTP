...
app.config['SQLALCHEMY_DATABASE_URI'] = (
    'sqlite:///' + os.path.join(basedir, "chatbox.db")
)
db.init_app(app)
...
# All courses route, showing from for each course
...
def all_courses(ref):
    ...
    # creates a list of forms for each course iterates through all courses and
    # creates one instance of 'enroll form for each course'
    # so we have a form for each course
    enroll_forms = [forms.EnrollForm() for _ in all_courses]

    if 'submit_enroll' in request.form and request.method == 'POST':
        ...
        for i, enroll_form in enumerate(enroll_forms):
            # Only validate the form that was submitted
            # (matches submitted_course_id)
            if str(all_courses[i].id) == submitted_course_id:
                if enroll_form.validate_on_submit():
                    ...
                    if joining_code_input == course.joining_code:
                        ...
                        if course in user.courses:
                            ...
                            db.session.add(user)
                            db.session.commit()
                            flash(f"Successfully enrolled in {course.subject}",
                                  'success')
                        return redirect(url_for('my_courses', ref=ref))
                    else:
                        flash(f"Invalid joining code for {course.subject}",
                              'error')
                else:
                    flash(f"There was a problem with your enrollment for \
                          {course.subject}.", 'error')

        # Handle search form submission
    if 'submit_search' in request.form and request.method == 'POST':
        if search_form.validate_on_submit():
            search_query = search_form.search.data
            all_courses = (
                models.Course.query
                .filter(models.Course.subject.like(f'%{search_query}%'))
                .all()
            )
            if not all_courses:
                flash("No courses found matching your search", 'error')
                # rename all_courses so it shows all the courses, ease of use
                all_courses = models.Course.query.all()
        else:
            flash('please enter a text between 2 and 60 characters', 'error')
...
def create_acc_teacher():
    ...
    if request.method == 'POST':
        if form.validate_on_submit():
            # check if the email used was used before
            ...
            flash('Account created successfully! \
                  Please login with your new details.', 'success')
            return redirect(url_for('login'))
        else:
            flash('Errors have occurred in form submission.', 'error')

    return render_template('create_acc_teacher.html', form=form,
...
def create_acc_student():
    ...
    if request.method == 'POST':
        if form.validate_on_submit():  # Check if form is valid
            # check if email being used is unique
            existing_user = models.User.query.filter_by(email=form.email.data).first()
            if existing_user:
                flash('Email already in use. Please choose a different one.', 'error')
        ...
        if request.method == 'POST':
            if form.validate_on_submit():
                ...
                new_course = models.Course(
                    # Generate a random joining code
...
def leave_course(ref, course_id):
    ...
    try:
        db.session.commit()
        flash(f'You have successfully left the course: {course.subject}',
              'success')
    except Exception as e:
        db.session.rollback()
        logging.error(f"Error while leaving the course: {str(e)}")
